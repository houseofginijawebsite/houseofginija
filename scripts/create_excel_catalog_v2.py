...
def populate_sheet(worksheet, workbook, product_list, formats, thumb_dir):
    headers = [
        'Product ID',
        'Image 1',
        'Image 2',
    ...
    # Enable Autofilters
    worksheet.autofilter(0, 0, len(product_list), len(headers) - 1)

    row_height = 85 # points

    for row_idx, p in enumerate(product_list, start=1):
        ...
        # 1. Product ID
        worksheet.write(row_idx, 0, pid, formats['cell_center'])
        ...
        for img_slot in range(5):
            ...
            if img_slot < len(imgs) and imgs[img_slot]:
                ...
                if os.path.exists(thumb_path):
                    worksheet.write(row_idx, col_pos, '', formats['cell_center'])
        ...
        # 3. Product Name
        worksheet.write(row_idx, 6, name, formats['cell_left'])

        # 4. Description
        worksheet.write(row_idx, 7, desc, formats['cell_left'])

        # 5. Original Price
        worksheet.write_number(row_idx, 8, orig_price, formats['price'])

        # 6. Discount %
        if discount_pct is not None and discount_pct > 0:
            worksheet.write_number(row_idx, 9, discount_pct, formats['percent'])
        else:
            worksheet.write(row_idx, 9, '', formats['cell_center'])

        # 7. Discounted Price
        worksheet.write_number(row_idx, 10, disc_price, formats['price'])

        # 8. Tags
        worksheet.write(row_idx, 11, tags_str, formats['cell_left'])

        # 9. Category
        worksheet.write(row_idx, 12, category, formats['category'])

def create_workbook_file(filename, all_products_sorted, untagged_products_sorted):
    workbook = xlsxwriter.Workbook(filename)
    formats = {
        'header': workbook.add_format({
            'bold': True,
            'bg_color': '#4A2E35', # Elegant brand plum
            'font_color': '#FFFFFF',
            'font_name': 'Calibri',
            'font_size': 11,
            'align': 'center',
    ...
    # Sheet 1: All Products (Sorted by Category)
    ws1 = workbook.add_worksheet('All Products (Sorted)')
    populate_sheet(ws1, workbook, all_products_sorted, formats, thumb_dir)

    # Sheet 2: Untagged Products (Blank in Tags section)
    ws2 = workbook.add_worksheet('Untagged Products')
    populate_sheet(ws2, workbook, untagged_products_sorted, formats, thumb_dir)
...
def build_catalog_excel():
    ...
    for p in products:
        p['_category'] = get_product_category(p)
        p['_tags_str'] = format_tags(p.get('tags'))

    # Sort key: Category first, then product name
    category_order = {
        'Suit (Kurta)': 1,
        'Drape Saree': 2,
        'Gown': 3,
    ...
    def sort_by_category_and_name(p):
        cat_rank = category_order.get(p['_category'], 99)
        name = p.get('name', '').lower()
        return (cat_rank, p['_category'], name)

    all_products_sorted = sorted(products, key=sort_by_category_and_name)
    untagged_products_sorted = [p for p in all_products_sorted if not p['_tags_str']]

    # Try writing to Boutique_Product_Catalog.xlsx
    primary_file = 'Boutique_Product_Catalog.xlsx'
    try:
        if os.path.exists(primary_file):
            os.remove(primary_file)
        create_workbook_file(primary_file, all_products_sorted, untagged_products_sorted)
    except PermissionError:
        print(f"Notice: '{primary_file}' is currently open in Excel. Writing to 'Boutique_Product_Catalog_Sorted.xlsx'...")
        create_workbook_file('Boutique_Product_Catalog_Sorted.xlsx', all_products_sorted, untagged_products_sorted)
